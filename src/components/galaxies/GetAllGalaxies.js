...
const GetAllGalaxies = ()=>{
    ...
    useEffect(()=>{
        //Make the API call
        fetch("https://mern3-node-mongo.onrender.com/fetchGalaxies")
        .then((res)=>{
            return res.json();
        }).then((datas)=>{
            setGalaxies(datas);
    ...
    },[idx,galaxies]);//Dependencies must be given to change the image on id
    ...
    const handleDelete = ()=>{
        ...
        setDialogContent(`Selected ${galaxies[idx].name} Item Deleted!!`);
        setProps(isDel);
        ...
        fetch(`https://mern3-node-mongo.onrender.com/deleteGalaxy?name=${galaxies[idx].name}`,{
            method: 'DELETE',
            headers: {
        ...
        switch(msg){
            case "fav":{
                if(!isFav){
                    ...
                    setProps(isFav);
                    setDialogTitle("Info");
                    setDialogContent("Galaxy Added to Favourites.. :)");
                }else{
                    //Now someone again tapping on fav so removing it from favourites
                    setProps(isFav);
                    setDialogTitle("Alert");
                    setDialogContent("Galaxy Removed Favourites!! :(");
                }
                break;
            }
            case "cart":{
                if(!isCart){
                    ...
                    setProps(isCart);
                    setDialogTitle("Info");
                    setDialogContent("Galaxy Added to Cart.. :)");
                }else{
                    //Now someone again tapping on fav so removing it from favourites
                    setProps(isCart);
                    setDialogTitle("Alert");
                    setDialogContent("Galaxy Removed Cart!! :(");
                }
                break;
            }
            case "book":{
        ...
        <div className="container-fluid">
            <header className="m-1 p-1 text-center text-white" style={{border:"2px solid white",backgroundColor:"black"}}>
                <h4 className="fs-7 font-monospace fw-bold fst-italic">
                    <AutoAwesome/> All Galaxy Related Information are Displayed Here
                </h4>
            </header>
                ...
                <Card style={{backgroundColor: isTheme ? "black" : "white", color : isTheme ? "white" : "black"}} className="mt-3 mb-2" elevation={4} >
                    ...
                    <CardContent className="row card-body">
                        <Typography className="font-monospace mb-1">
                            Type: <b className="font-monospace">{galaxies[idx].type}</b><br/>
                            Distance: <b className="font-monospace">{galaxies[idx].distance}</b><br/> 
                        </Typography>
