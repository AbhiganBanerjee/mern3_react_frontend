...
const GetAllBooks = ()=>{
    ...
    useEffect(()=>{
        //Make the API call
        fetch("https://mern3-node-mongo.onrender.com/fetchBooks")
        .then((res)=>{
            return res.json();
        }).then((datas)=>{
            setBooks(datas);
    ...
    },[idx,books]);//Dependencies must be given to change the image on id
    ...
    const handleDelete = ()=>{
        ...
        setDialogContent(`Selected ${books[idx].name} Item Deleted!!`);
        setProps(isDel);
        ...
        fetch(`https://mern3-node-mongo.onrender.com/deleteBook?name=${books[idx].name}`,{
            method: 'DELETE',
            headers: {
    ...
    return(
        <div className="container-fluid">
            <header className="m-1 p-1 text-center" style={{border:"2px solid #CC313D",backgroundColor:"#F7C5CC",color:"#CC313D"}}>
                <h4 className="fs-7 font-monospace fw-bold fst-italic">
                    <span className="bi bi-book-half"></span> All Books Related Information are Displayed Here
                </h4>
            </header>
            <Container component="main" maxWidth="xs">
                {/* Start the Card */}
                <Card style={{backgroundColor: !isTheme ? "#F7C5CC" : "#CC313D",color: !isTheme ? "#CC313D" : "#F7C5CC"}} className="mt-3 mb-2" elevation={4} >
                    {/* Create the Card Header */}
                    <CardHeader 
                        avatar = {
                            <Avatar 
                                style={{border:!isTheme ? "2px solid #CC313D" : "2px solid #F7C5CC" ,borderRadius:"50%"}} 
                                // data:image/jpeg;base64,{{ base64Image }}
                                src={`data:image/jpeg;base64,${books[idx].image}`}
                            />
                        }
                        action = {
                        ...
                        title = {<b className="fs-5 font-monospace fw-bolder fst-italic">{books[idx].name}</b>}
                    />
                    {/* Card Body */}
                    <CardContent className="row card-body">
                        <Typography className="font-monospace mb-1">
                            Price: <b className="font-monospace">{books[idx].price}</b><br/>
                            Author: <b className="font-monospace">{books[idx].author}</b><br/> 
                        </Typography>
                        ...
                        <div className="col-10 d-flex flex-column">
                            ...
                            <CardMedia 
                                ...
                                image={`data:image/jpeg;base64,${books[idx].image}`}
                                style={{border: !isTheme ? "4px solid #CC313D" : "4px solid #F7C5CC"}}
                                height="340"
                            />
                            <b className="font-monospace text-center fst-italic" id="status" style={{display:"none",color: !isTheme ? "#CC313D" : "#F7C5CC"}}></b>
                        </div>
                        <CardActions className="col-1 d-flex justify-content-center flex-column">
                    ...
                    {/* Card Footer */}
                    <div className="row" style={{borderTop: !isTheme ? "2px solid #CC313D" : "2px solid #F7C5CC"}}>
