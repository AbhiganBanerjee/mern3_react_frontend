...
const GetAllAnimes = ()=>{
    ...
    useEffect(()=>{
        //Make the API call
        fetch("https://mern3-node-mongo.onrender.com/fetchAnimes")
        .then((res)=>{
            return res.json();
        }).then((datas)=>{
            setGames(datas);
    ...
    const handleDelete = ()=>{
        ...
        fetch(`https://mern3-node-mongo.onrender.com/deleteAnime?name=${games[idx].name}`,{
            method: 'DELETE',
            headers: {
    ...
    return(
        <div className="container-fluid">
            <header className="m-1 p-1 text-center" style={{border:"2px solid maroon",backgroundColor:"green",color:"maroon"}}>
                <h4 className="fs-7 font-monospace fw-bold fst-italic">
                    <span className="bi bi-badge-4k-fill"></span> All Animes Related Information are Displayed Here
                </h4>
            </header>
                ...
                <Card style={{backgroundColor:!isTheme ? "green" : "maroon",color:!isTheme ? "maroon" : "green"}} className="mt-3 mb-2" elevation={4} >
                    ...
                    <CardContent className="row card-body">
                        <Typography className="font-monospace mb-1">
                            ...
                            Author: <b className="font-monospace">{games[idx].author}</b><br/> 
                        </Typography>
